...
\section{Skip Length}\index{Skip Length}
  ...
  \setting{Skip Length} changes the behaviour of the \ActionWpsSkipNext{}
  button so that it skips by a given time instead of skipping to a new track.
  The \setting{Skip to Outro} option changes the behaviour so that the button
  skips to just before the end of the track, so that the last few seconds are
  played before the next track.

\section{Skip Length Backwards}\index{Skip Length Backwards}
  Similar to \setting{Skip Length}, but for the \ActionWpsSkipPrev{} button.
  The \setting{Same as Forward} option sets the backwards direction skip length
  to skip the same amount as the forward-direction \setting{Skip Length} specifies.
